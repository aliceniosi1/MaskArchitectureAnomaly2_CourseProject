...
# -----------------------------------------------------------------------------
@torch.no_grad()
def anomaly_msp_from_pixel_scores(pixel_scores_chw: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    denom = pixel_scores_chw.sum(dim=0, keepdim=True).clamp_min(eps)
    probs = pixel_scores_chw / denom
...
def main():
    ...
    parser.add_argument("--out_csv", type=str, default=None, help="Path del CSV di output. Se non specificato, non salva nulla.")
    ...
    for T in temps:
        ...
        "best_T_by_auprc": float(best_T) if best_T is not None else "", })
    ...
    if args.out_csv is not None:
        ...
        fieldnames = [
            "temperature", "auprc_percent", "fpr95_percent",
            "n_ind_pixels", "n_ood_pixels",
            "images_matched", "images_processed", "images_without_ood", "errors",
            "weights", "img_size_h", "img_size_w", "best_T_by_auprc"
        ]
